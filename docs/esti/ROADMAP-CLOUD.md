...
its enqueue boundary — `gateway/` — is built and live-verified with all 10
buildable render targets wired end to end (drawings' issue-set PDF is the
one deliberate exception — its upload pipeline doesn't exist yet, so
there's no svg_key any button could ever act on); what's left is the real
VPS deploy step, not an open architecture question or more screens to wire)  
**Updated:** 2026-09-05  

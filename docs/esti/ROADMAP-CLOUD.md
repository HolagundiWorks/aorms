...
- **Unreachable code, needs a decision first (found during the Phase 9
  audit)** — `backend/src/modules/pmc/contractorPortal.ts` exports a
  `contractorPortalRouter` that is never imported anywhere; the namespace
  actually mounted in `trpc/router.ts` is the *other*
  `contractorPortalRouter`, from `contractor/portal.ts`. Unlike the two
  items above this isn't a clean delete: the dead file is AProc
  package-level sealed-bid *submission* — a contractor invited to bid on
  an AProc package currently has no working API path to submit that bid
  at all. Decide first (resurrect it under a different export name and
  wire it in, or confirm the feature is intentionally gone and delete it),
  then act — see [NEXTJS-MIGRATION-PHASE9-AUDIT.md](./NEXTJS-MIGRATION-PHASE9-AUDIT.md).

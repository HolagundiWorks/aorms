...
**The single roadmap for this repo** — merged 2026-09-10 from three
previously separate files (`ROADMAP.md`'s thin index, `ROADMAP-CLOUD.md`,
`ROADMAP-LOCAL.md`), on explicit direction to consolidate and cut stale
content rather than maintain three drifting documents. `ROADMAP-CLOUD.md`
and `ROADMAP-LOCAL.md` now redirect here — see them if something still
links to the old paths.

**What got cut, and why:** both source docs carried large sections
entirely about the **old VPS-deployed stack** (React/MUI `frontend` +
Fastify `backend` + raw PostgreSQL) — a soft-launch status table gated by
`VITE_MARKETING_ONLY`, that stack's own CI/build-health snapshot, its
Docker Compose/VPS infrastructure table, and a local Podman dev-loop
("`podman compose up -d --build`", local Postgres, local Supabase
stacks). All of that is now genuinely stale, not just old: `web/` is the
live production site as of 2026-09-09 (see History below), the old
`backend`/`worker` don't even run locally anymore (`drizzle-orm` and its
Postgres dependencies were removed 2026-09-05 — see `CLAUDE.md` § Dev/
verify loop), and both local Supabase stacks + the Podman VM they ran in
were removed entirely 2026-09-08. Keeping those sections would mean
presenting a workflow that no longer exists as if it were current — cut
outright rather than carried forward. The detailed **dated build
narrative** underneath all of that (every migration, every incident,
every verified fix — the real engineering record) was **not** cut; it's
preserved in full below under History, just no longer mixed in with
stale status tables at the top of the file.
...
2. **AORMS Platform** (`web/app/(platform)/*`, a separate Supabase
   ...
   material/interior suppliers (`AORMS-C-`, branded **ConnectDeX
   Partners** on the public landing page as of 2026-09-10 — a marketing
   name only, not a system rename; see `docs/esti/AORMS-PLATFORM-
   ARCHITECTURE.md`), plus per-Studio Trial/Standard/Premium licensing
   with real Razorpay payments and a platform-staff admin back office
   (`/admin/*`).
3. **Public marketing site** (`web/app/page.tsx` and friends) — the
   landing page, a minimal markdown blog, SEO (sitemap, robots.txt, a
   real OG image, structured data), and a public read-only demo account.

See **History** below for the full, dated account of how each of these
was built and verified — this Status section is deliberately just the
current snapshot, not the story.

---

## What's live now

**Office Hub** — Clients & Leads (CRM, tender tracking, portal access) ·
Projects (phases, tasks, milestones, moodboards, delivery tracking) ·
Proposals & Contracts (unified proposals, client approval gates,
versioning) · Invoicing & Finance (GST-compliant invoicing,
reconciliation, cash book, financial reports/filing abstracts) · Delivery
& Supervision (BBS/IS 456, steel reconciliation, running bills, site
supervision) · Team & HR (roster, assignments, leaves, payroll, ASPRF
scoring, attendance) · Knowledge Bank (spec catalogue, compliance
library, standards, master plans, lessons learned) · ESTI AI (read-only
Q&A grounded in a live-count snapshot, plus AI Studio document drafting —
9 of the original 14 draft kinds, the ones with a real `web/` data
source).

**AORMS Platform** — Identity (`/identity`, portable `AORMS-U-` accounts,
automatic Basic→Pro at 100 usage hours) · Studios (`/studios/[id]`,
architecture-practice accounts, team/role management) · Companies/
ConnectDeX Partners (`/companies/[id]`, material-supplier accounts with a
product catalogue) · Materials directory (`/materials`, cross-entity
nearest-first product discovery) · Licensing (`/licences`, Trial/
Standard/Premium via real Razorpay checkout — see History, 2026-09-09/10
entries) · Admin back office (`/admin/*`, platform-staff-only: dashboard,
licence overrides, payments, pricing, activity log — gated by a real
`is_admin` column, settable only via direct DB access).

**Public site** — Landing page (header with mobile nav, hero, Brief,
Specification, Intelligence, Fee Proposal, Blog, Identity, ConnectDeX
Partners, CTA band with a public demo, FAQ, footer) · Blog (`/blog`,
markdown files, no CMS) · SEO (`/sitemap.xml`, `/robots.txt`, a real
1200×630 OG image, JSON-LD structured data) · Public demo account
(`demo@aorms.in`, VIEWER role — genuinely read-only, RLS-enforced — Office
Hub data reset nightly via `pg_cron`).
...
- **Correction (2026-09-21): the `clients` migration this entry below
  claimed was "never actually committed" WAS committed** — this branch
  was cut from a checkout point before `0082_clients_write_requires_
  capability.sql` landed on `main` (commit `34143883`, same day), so it
  genuinely didn't exist yet from that branch's point of view; it wasn't
  a false "written, not yet applied" claim. Renumbered this branch's own
  migration `0081_write_policies_require_capability.sql` → `0085` on
  merge, since `main` had since gained real `0081`-`0084` migrations of
  its own (`provision_firm_requires_platform_identity`, `clients`,
  `contractors`, `project_offices` — the last two independently closing
  the same VIEWER-write bug class this migration's `contractors` and
  `project_offices`/`phases`/`tasks` entries also cover; harmless
  redundant `alter policy` statements, not a conflict). Original entry,
...
- ESTI AI agent fully live on the office hub (task recommendations,
  project health insights, email draft automation, reminders)
...
- **AORMS Platform nomenclature/boundaries** (Studio vs. Company vs.
  Account, Portal vs. Admin vs. Directory) — [AORMS-PLATFORM-
  ARCHITECTURE.md](./AORMS-PLATFORM-ARCHITECTURE.md)
- **Next.js/Supabase migration target spec** — [NEXTJS-SUPABASE-
  MIGRATION.md](./NEXTJS-SUPABASE-MIGRATION.md)
- **Hostinger deployment** (env vars, build/start commands, known
  incidents) — [WEB-DEPLOY-HOSTINGER.md](./WEB-DEPLOY-HOSTINGER.md)
- **Portable identity design** (`AORMS-U-`/`AORMS-C-` handles, the
  original design this system was built from) —
  [AORMS-IDENTITY.md](./AORMS-IDENTITY.md)
- **Cloud-agent branch workflow rules** —
...
**Three roadmap files merged into one, stale content cut (2026-09-10), on
explicit direction.** This repo had three separate roadmap documents —
`docs/esti/ROADMAP.md` (a thin index pointing at the other two),
`ROADMAP-CLOUD.md` (this History section, plus several status tables
about the old VPS stack), and `ROADMAP-LOCAL.md` (a local Podman dev-loop
guide plus a Carbon-migration/cleanup tracker). Read all three in full
before changing anything, rather than guessing at what was safe to cut.

**What got merged in unchanged**: this entire History section — every
dated entry, verbatim, starting from `## Stack migration — Next.js +
Supabase` above — moved here from `ROADMAP-CLOUD.md` with only its own
heading retitled. Nothing in the actual narrative was edited, trimmed, or
rewritten; the value of a dated build log is in it being an accurate
at-the-time account, and 3776 lines of nuanced technical history isn't
something to hand-edit for "staleness" without real risk of introducing
errors — a stale *status snapshot* and a stale *historical record* are
different things, and only the former needed cutting.

**What got cut outright, not carried forward**: `ROADMAP-CLOUD.md`'s own
top-of-file status block (describing 2026-09-08 state, before the
Hostinger cutover, the admin/payments system, or any of the landing-page
work existed) and three of its section headers — `## Current phase —
soft launch` (a `VITE_MARKETING_ONLY` gate table for the now-retired old
frontend), `## CI / build health` (that same old stack's own CI
snapshot), `## Cloud infrastructure` (its Docker Compose/VPS
infrastructure table) — all three describe a stack this repo no longer
tracks as current (`web/` has been the live production site since
2026-09-09). From `ROADMAP-LOCAL.md`: its entire local Podman dev-loop
section (`podman compose up -d --build`, local Postgres, local Supabase
stacks) — genuinely dead instructions, since local Postgres was removed
2026-09-04, `backend`/`worker`'s Postgres/Drizzle dependencies were
removed 2026-09-05 (neither runs locally anymore at all), and both local
Supabase stacks + the Podman VM they ran in were removed 2026-09-08; and
its already-fully-checked-off "office system pivot cleanup" phase tracker
(Phases 1–3), which documented work long finished with zero forward value.

**What got folded in, honestly flagged as uncertain rather than reasserted
done or freshly stale**: `ROADMAP-LOCAL.md`'s Carbon Design System
migration Wave 3+ status and its old-stack Phase 4 cleanup checklist
(installer env vars, Tauri dependencies, allied-app API endpoints) — both
moved into the new Open Items section at the top of this file with an
explicit "last known state, not reverified since" caveat, since this
session did not check either.

`ROADMAP-CLOUD.md` and `ROADMAP-LOCAL.md` are now short redirect stubs at
their same paths (≈39 other files across the repo still link to them
directly — SQL migration comments, `worker/` Python code, phase-audit
docs — none of those were hunted down and fixed in this pass; the stubs
exist specifically so those links keep resolving to something useful
rather than a dead file, without requiring a much larger, separately-
scoped cross-reference cleanup). `CLAUDE.md`'s own ~12 references and
`AORMS-PLATFORM-ARCHITECTURE.md`'s 2 were updated directly, being the
two most actively-maintained cross-referencing docs.

Verified: read all three source files in full before cutting anything;
confirmed the merged file's section boundaries render as clean markdown
(no header collisions, no broken transitions) via direct inspection at
each seam; confirmed via `grep` that no stale `ROADMAP-CLOUD.md`/
`ROADMAP-LOCAL.md` references remain in `CLAUDE.md`. Not run through
...
- **Deploying / what's live now?** See Status and What's live now above,
  or [WEB-DEPLOY-HOSTINGER.md](./WEB-DEPLOY-HOSTINGER.md) for the
  Hostinger-specific mechanics.
- **Product definition?** See [AORMS-OFFICE-SYSTEM.md](./AORMS-OFFICE-SYSTEM.md)
- **Platform nomenclature / Studio vs. Company vs. Admin boundaries?** See
  [AORMS-PLATFORM-ARCHITECTURE.md](./AORMS-PLATFORM-ARCHITECTURE.md)
- **Stack migration spec?** See [NEXTJS-SUPABASE-MIGRATION.md](./NEXTJS-SUPABASE-MIGRATION.md)
  · [Phase 2 audit](./NEXTJS-MIGRATION-PHASE2-AUDIT.md) · [Phase 3 audit](./NEXTJS-MIGRATION-PHASE3-AUDIT.md)
  · [Phase 4 audit](./NEXTJS-MIGRATION-PHASE4-AUDIT.md) · [Phase 5 audit](./NEXTJS-MIGRATION-PHASE5-AUDIT.md)
  · [Phase 6 audit](./NEXTJS-MIGRATION-PHASE6-AUDIT.md) · [Phase 7 audit](./NEXTJS-MIGRATION-PHASE7-AUDIT.md)
  · [Phase 8 audit (proposed, not adopted)](./NEXTJS-MIGRATION-PHASE8-AUDIT.md)
  · [Phase 9 audit (proposed, not adopted)](./NEXTJS-MIGRATION-PHASE9-AUDIT.md)
  · [Phase 10 audit (proposed, not adopted)](./NEXTJS-MIGRATION-PHASE10-AUDIT.md)

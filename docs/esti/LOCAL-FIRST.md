...
| **LF3** | Domain enqueue of metadata (tasks, estimate totals, phase progress) + apply hooks on pull | ✅ enqueue on node writes (`domainMeta.ts` + task / estimate / phaseProgress routers); apply merge map on `sync.pullMeta` |
| **LF4** | Signed desktop installer (Tauri + profile STUDIO\|CONSULTANCY); first-run licence bind | 🚧 **unsigned** Studio Setup.exe built overnight (`desktop/artifacts/`) · panel `syncToken` · `DesktopLicenceBind` · [HUB-API.md](HUB-API.md); **sign + physical bind** = morning ([MORNING-TEST-LF4.md](MORNING-TEST-LF4.md)) |
| **LF5** | Web parity polish: capability badges, degraded AI UX, shared keymap / Help | ✅ 2026-08 |
| **LF6** | UX parity checklist + inspector/AI right-slot; Figma token sync to kit | ✅ partial — U-BW staff chrome; Figma sync open |
...
- [HUB-API.md](HUB-API.md) — versioned hub auth / licence / sync contract  
- [ROADMAP.md](ROADMAP.md) § Local-first · § Web Portal  
- [WEB-PORTAL.md](WEB-PORTAL.md) — download paths for desktop installers  
- [DESKTOP-REPOS.md](DESKTOP-REPOS.md) — Studio/Consultancy extraction gate  
- [HCW-LICENSE-MANAGER.md](HCW-LICENSE-MANAGER.md) — `/platform/v1` authority  

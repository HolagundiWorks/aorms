...
| Tenancy | **Multi-tenant as of 2026-09-19** (`public.firms`, one row per Studio; every tenant table carries `firm_id`, every RLS policy checks it via `current_firm_id()` — see CLAUDE.md's dated "Third stale-doc correction" for the full account, migrations `0053`–`0068`). A profile's `firm_id` is its *currently active* firm, switchable via `/select-studio`; was a hard Postgres singleton before this date. | Multi-tenant by design since inception — many Studios/Companies under one platform database, each with many-people-per-entity membership |

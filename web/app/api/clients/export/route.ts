...
export async function GET(): Promise<NextResponse> {
  ...
  const { data: clients, error } = await supabase.from("clients").select("name, kind, city, email, phone").order("name");

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  const csv = toCsv(
    (clients ?? []).map((c) => ({ Name: c.name, Type: c.kind, City: c.city, Email: c.email, Phone: c.phone })),
    COLUMNS,
  );

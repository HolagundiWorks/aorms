-- Extensive demo data (2026-09-13) — explicit direction: a firm with 5
-- real staff, 12 projects, a real client/contractor/consultant roster,
-- and task/invoice/proposal/lead/decision variety covering most real
-- case scenarios, so testing (and the app's own demo login) has
-- something substantial to click through instead of the 2-project/
-- 5-task placeholder this function used to write. Same function name/
-- signature as before (`reset_demo_data()`, no args) so the existing
-- nightly `pg_cron` job (migration — see ROADMAP.md's dated entry;
-- jobid 1, `30 21 * * *`) keeps calling it unchanged.
--
-- The 5 staff (aditi.rao/vikram.shah/akash.mehta/priya.nair/
-- rohan.desai@aorms.in) are real auth.users + profiles created once via
-- the Auth Admin API this session, NOT recreated by this function —
-- only the transactional data below (clients/projects/tasks/invoices/
-- proposals/leads/decisions/moms/progress_reports/contractors/
-- consultants/leaves) is reset-and-reseeded on every call, same
-- delete-then-insert idempotency the old version already had, just
-- across many more tables. If those 5 accounts don't exist yet (e.g. a
-- fresh project rebuild), this function returns early exactly like the
-- old demo@aorms.in guard did — it's additive data, not a hard
-- dependency of anything else in the schema.

...
/**
 * The owner-editable form for a studio's regulatory/contact profile —
 ...
 *
 * The caller MUST pass a `key` derived from `studio`'s own fields (see
 * app/(platform)/studios/[studioId]/page.tsx) — every field here is an
 ...
 */
export function StudioProfileForm({ studio }: { studio: StudioProfile }) {
  const [state, formAction, pending] = useActionState<PlatformActionState, FormData>(updateStudioProfile, null);
      ...
      <Stack gap={5}>
        ...
        <FormGrid>
          <TextInput
            ...
            defaultValue={studio.coa_registration_no ?? ""}
          />
          <TextInput id="email" name="email" labelText="Email" type="email" defaultValue={studio.email ?? ""} />
          <TextInput id="phone" name="phone" labelText="Phone" defaultValue={studio.phone ?? ""} />
        </FormGrid>

        <h3 className="cds--type-productive-heading-02">GST & Tax</h3>
        <FormGrid>
          <Select id="gstType" name="gstType" labelText="GST system" defaultValue={studio.gst_type}>
          ...
          <TextInput id="gstin" name="gstin" labelText="GSTIN" defaultValue={studio.gstin ?? ""} />
          <TextInput id="pan" name="pan" labelText="PAN" defaultValue={studio.pan ?? ""} />
        </FormGrid>
        <Checkbox
          id="tdsApplicableDefault"
          name="tdsApplicableDefault"
          labelText="Deduct TDS (s.194J) by default"
          defaultChecked={studio.tds_applicable_default}
        />
        ...
        <FormGrid>
          <TextInput id="addressLine1" name="addressLine1" labelText="Address line 1" defaultValue={studio.address_line1 ?? ""} />
          <TextInput id="addressLine2" name="addressLine2" labelText="Address line 2" defaultValue={studio.address_line2 ?? ""} />
          <TextInput id="city" name="city" labelText="City" defaultValue={studio.city ?? ""} />
          <TextInput id="district" name="district" labelText="District" defaultValue={studio.district ?? ""} />
          <Select id="state" name="state" labelText="State" defaultValue={studio.state ?? ""}>
            <SelectItem value="" text="— Select a state —" />
            {STATE_NAMES.map((s) => (
              <SelectItem key={s} value={s} text={s} />
            ))}
          </Select>
          <TextInput id="pincode" name="pincode" labelText="PIN code" defaultValue={studio.pincode ?? ""} />
        </FormGrid>

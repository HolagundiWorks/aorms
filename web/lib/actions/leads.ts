 ...
 * (`leads: staff write`, migration 0085_write_policies_require_capability.sql).
 * Added 2026-09-21 (QA finding: `/leads`' "Add lead" button was visible and
 * fully interactive for a VIEWER — the RLS policy already blocked the
...
export async function createLead(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!authProfile || !WRITE_TIER_ROLES.has(authProfile.role)) {
    return { error: "You don't have permission to add leads — contact a firm owner or partner." };
  }

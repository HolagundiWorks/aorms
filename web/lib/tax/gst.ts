/**
 * India GST profile — hardcoded, non-configurable. Verbatim port of
 * packages/contracts/src/gst.ts (see docs/esti/INDIA-PROFILE.md for the
 * design rationale). The firm operates under exactly one GST system at a
 * time, read from `firm.gst_type`.
 */
import { pct, roundToRupee, type Paise } from "./money";
 ...
 *   client's registered address — see `derivePlaceOfSupply()` in
 *   ./place-of-supply, which derives it.
 */
